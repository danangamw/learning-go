...
func TestValidateLength(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			ssn:    "1234567890",
			wanted: fmt.Errorf("the value of %s caused an error: %v\n", "1234567890", ErrInvalidSSNLength),
		},
	}
...
func TestIsNumber(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			ssn:    "12345678r",
			wanted: fmt.Errorf("the value of %s caused an error: %v\n", "12345678r", ErrInvalidSSNNumbers),
		},
	}
...
func TestIsDigitPlace(t *testing.T) {
	testCases := []struct {
		...
		{
			name:   "Test SSN valid digit placement of 9",
			ssn:    "923956789",
			wanted: nil,
		},
		{
			name:   "Test SSN valid valid digit placement of 7",
			ssn:    "923756789",
			wanted: nil,
		},
	}
